...
set -e
#
# Modified from the AMRFinder installation script
# Citation to original file here: https://github.com/ncbi/docker/tree/master/amr
#
...
docker run --rm lociq bash -c 'cd /usr/local/bin; ./test_amrfinder.sh'
if [ $? -gt 0 ]
then
    >&2 echo "ERROR! Tests for AMRFinder within Lociq failed"
    exit 1
else
